...
class TestDatabaseEngine:
    ...
    def test_set_task_vm_and_guest_start(self, db: _Database, freezer):
        with db.session.begin():
            assert db.set_task_vm_and_guest_start(1, "vmname", "vmlabel", 1, "kvm") is None
            task_id = db.add_url("https://www.google.com")
            machine = self.add_machine(db)
            db.session.expunge_all()
        with db.session.begin():
            guest_id = db.set_task_vm_and_guest_start(task_id, machine.name, machine.label, machine.id, "KVM")
    ...
    def test_guest_get_status(self, db: _Database):
        with db.session.begin():
            task_id = db.add_url("https://www.google.com")
            machine = self.add_machine(db)
            db.set_task_vm_and_guest_start(task_id, machine.name, machine.label, machine.id, "KVM")
        with db.session.begin():
