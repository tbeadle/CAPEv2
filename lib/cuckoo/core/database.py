...
class _Database:
    ...
    def list_machines(
        self, locked=None, label=None, platform=None, tags=None, arch=None, include_reserved=False, os_version=None
    ) -> List[Machine]:
    ...
    def lock_machine(self, label=None, platform=None, tags=None, arch=None, os_version=None):
    ...
    def check_file_uniq(self, sha256: str, hours: int = 0):
        # This function is poorly named. It returns True if a sample with the given sha256
        # already exists in the database, rather than returning True if the given sha256
        # is unique.
        uniq = False
        if hours and sha256:
    ...
    def list_sample_parent(self, sample_id=False, task_id=False):
        ...
        if sample_id:  # pragma: no cover
            ...
            try:
                ...
                # If nothing is found, then None is returned, which can't be unpacked.
                pass
    ...
    def view_machine(self, name):
        ...
        # machine = self.session.execute(select(Machine).filter(Machine.name == name).options(joinedload(Tag))).first()
        return machine
